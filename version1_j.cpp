...
{
     cout<<endl<<"-->States=anand:"<<p_anand<<"||z:"<<p_z;
...
int checkCellDup(int p_anand,int p_z,int p_t)
{
     int check = -1;
     for(int y=0;y<p_z;y++)
     {
             if(listArr[p_anand][y] == listArr[1][p_t])
                check = 1;
     }
     if(check =1) return false;
     else if(check == -1) return true;
}
int createCell(int p_anand,int p_z)
{
     list<char>::iterator itt = listArr[p_anand][p_z].begin();
     while(*itt!='.') itt++;
     itt++;
     for(int t=0;t<nonTerm.size();t++)  //TRAVERSING THE NON TERMINAl LIST
     {
        if(*itt == nonTerm[t])		//IF A NON TERMINAL IS FOUND
        {
        	cout<<endl<<"Found NonTerminal:"<<*itt;
        	cout<<"    Value of anand:"<<p_anand;
        	cout<<"    Value of z:"<<p_z;
        	cout<<"    VAlue of t:"<<t+1;
        	if(checkCellDup(p_anand,p_z,t+1))
        	{
                 p_z++;
                 listArr[p_anand][p_z].assign(listArr[1][t+1].begin(),listArr[1][t+1].end());  //COPY ELEMENTS FROM GOTO TO THE NEXT ANAND POSITION
        		n[p_anand]++; 
        		//CALL FUNCTION TO CHECK FOR NON TERMINALS IN RECENTLY CREATED CELLS
        		createCell(p_anand,p_z);
            }	
        }                            
     }
     return p_z;
                ...
                repeat.clear();
             	...
             	if(check_done == 1)
             	{
	                ...
	                for(int k=0;k<repeat.size();k++) //RUN ACCORDING TO NUMBER OF REPITITIONS
	            	{
	                	...
	                	{ cout<<endl<<"---->Going To Last Dott"; z++;goto lastdott;}
					    it++;
	            		z++;
